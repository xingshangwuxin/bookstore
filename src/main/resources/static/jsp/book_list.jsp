...
<%--                从bookPageInfo.list数组中得到一个对象bookInfo显示，开始循环--%>
                <c:forEach items="${bookPageInfo.list}" var="bookInfo">
                    <li>
                        ...
                        <p class="search_book_author">
                            <span>${bookInfo.author}著</span>
                            <span> /${bookInfo.publishDate}</span>
                            <span>  ${bookInfo.press}</span>
                        </p>
                        <p class="detail">
                                ${bookInfo.detail}
                        </p>
...
<%--                                <a class="search_btn_collect" href="javascript:void(0);">收藏</a>--%>
        ...
        <ul class="pagination pagination-lg">
            ...
            <c:if test="${!bookPageInfo.isFirstPage}">
                <li>
                    <a href="book/list?keywords=${keywords}&cateId=${cateId}&page=${bookPageInfo.prePage}">上一页</a>
                </li>
            </c:if>
...
<%--                    显示1、2，3，4，5，6页的按钮
                              从pagenum[begin]开始迭代，从pagenum[end]结束
--%>
            <c:forEach
            ...
            <%--
                下一页
            --%>
<%--                是最后一页的时候，什么也做不了，javascript:void(0)--%>
            <c:if test="${bookPageInfo.isLastPage}">
                <li class="disabled"><a href="javascript:void(0);">下一页</a></li>
            </c:if>
<%--                不是最后一页，就进入到controller层进行拦截判断--%>
            <c:if test="${!bookPageInfo.isLastPage}">
